...
- На данный момент, серверная часть работает корректно. То есть добавление нового, удаление элемента, перенос элемента и всех его детей - все это работает.   
- Функция `children_range()` в модуле `calc_values.py` - работает нормально.  
- Функция `parents_list()` в модуле `calc_values.py` - работает нормально только если количество детей, в размерности дерева, указано **нечетное**.  
